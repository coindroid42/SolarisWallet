...
import xlrtrum.exceptions.InvalidPeerVersion;
import xlrtrum.listeners.AddressListener;
import xlrtrum.listeners.PeerDataListener;
import xlrtrum.listeners.PeerListener;
...
public class XlrtrumPeergroup implements PeerListener, PeerDataListener {

    private static final Logger log = LoggerFactory.getLogger(XlrtrumPeergroup.class);
    ...
    /** Trusted peer */
    private XlrtrumPeer trustedPeer;
    // Currently active peers.
    private final CopyOnWriteArrayList<XlrtrumPeer> peers;
    // Currently connecting peers.
    private final CopyOnWriteArrayList<XlrtrumPeer> pendingPeers;
    ...
    public XlrtrumPeergroup(NetworkConf networkConf, WalletManager walletManager, AddressStore addressStore) throws IOException {
    ...
    public XlrtrumPeergroup(NetworkConf networkConf) throws IOException {
        this.peers = new CopyOnWriteArrayList<>();
        this.pendingPeers = new CopyOnWriteArrayList<>();
        this.networkConf = networkConf;
        this.ioManager = new IoManager(1,1);
    ...
    public synchronized void start() throws InterruptedException, ConnectionFailureException {
        try {
            log.info("Starting XlrtrumPeergroup");
            isActive = true;
            // todo: first part discovery..
            /*
            * Connect to the trusted node and get servers from it.
            */
            trustedPeer = new XlrtrumPeer(networkConf.getTrustedServer(), ioManager,versionMsg);
            ...
            isRunning = false;
            isActive = false;
            log.error("XlrtrumPeerGroup start",e);
            throw e;
        }
    }
    ...
    @Override
    public void onConnected(XlrtrumPeer xlrtrumPeer) {
        try {
            if (xlrtrumPeer == trustedPeer) {
                ...
                for (PeerListener peerConnectionListener : peerConnectionListeners) {
                    peerConnectionListener.onConnected(xlrtrumPeer);
                }
                ...
                for (InetSocketAddress inetSocketAddress : networkConf.getNetworkServers()) {
                    XlrtrumPeerData peerData = new XlrtrumPeerData(inetSocketAddress.getHostName(),inetSocketAddress.getPort(),0);
                    XlrtrumPeer peer = new XlrtrumPeer(peerData,ioManager,versionMsg);
                ...
                log.info("Non trusted peer connected, "+xlrtrumPeer.getPeerData());
                pendingPeers.remove(xlrtrumPeer);
                peers.add(xlrtrumPeer);
            }
        }catch (Exception e){
            e.printStackTrace();
        }
    ...
    @Override
    public void onDisconnected(XlrtrumPeer xlrtrumPeer) {

    }

    @Override
    public void onExceptionCaught(XlrtrumPeer xlrtrumPeer, Exception e) {
    ...
    @Override
    public void onSubscribedAddressChange(XlrtrumPeer xlrtrumPeer, String address, String status) {
        try {
    ...
    @Override
    public void onListUnpent(XlrtrumPeer xlrtrumPeer,String address, List<Unspent> unspents) {
    ...
    @Override
    public void onBalanceReceive(XlrtrumPeer xlrtrumPeer, String address, long confirmed, long unconfirmed) {
        try {
            if (xlrtrumPeer == trustedPeer) {
    ...
    @Override
    public void onGetHistory(XlrtrumPeer xlrtrumPeer, StatusHistory statusHistory) {
        try {
