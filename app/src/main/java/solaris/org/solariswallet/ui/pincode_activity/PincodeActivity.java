package solaris.org.solariswallet.ui.pincode_activity;
...
import solaris.org.solariswallet.ui.backup_mnemonic_activity.MnemonicActivity;
import solaris.org.solariswallet.ui.base.BaseActivity;
import solaris.org.solariswallet.ui.settings_pincode_activity.KeyboardFragment;
...
public class PincodeActivity extends BaseActivity implements KeyboardFragment.onKeyListener {
    ...
    protected void onCreateView(Bundle savedInstanceState, ViewGroup container) {
        ...
        if (solarisApplication.getAppConf().getPincode()!=null && !checkPin){
            goNext();
            finish();
        }
    ...
    private void goNext() {
        if (solarisApplication.getAppConf().getTrustedNode()==null){
            // select random trusted node
            List<XlrtrumPeerData> nodes = XlrtrumGlobalData.listTrustedHosts();
            Random random = new Random();
            solarisApplication.setTrustedServer(nodes.get(random.nextInt(nodes.size())));
            solarisApplication.stopBlockchain();
        }

        solarisApplication.getAppConf().setAppInit(true);
        ...
        if (lastPos<4) {
            if (key.getValue() < 10) {
                ...
                if (lastPos == 4) {
                    ...
                    if (!checkPin) {
                        solarisApplication.getAppConf().savePincode(pincode);
                        Toast.makeText(this, R.string.pincode_saved, Toast.LENGTH_SHORT).show();
                        goNext();
                    }else {
                        // check pin and return result
                        if(solarisApplication.getAppConf().getPincode().equals(pincode)){
