...
import static solaris.org.solariswallet.ui.transaction_detail_activity.FragmentTxDetail.IS_DETAIL;
import static solaris.org.solariswallet.ui.transaction_detail_activity.FragmentTxDetail.TX_WRAPPER;
import static solaris.org.solariswallet.utils.TxUtils.getAddressOrContact;
...
public class TransactionsFragmentBase extends BaseRecyclerFragment<TransactionWrapper> {
    ...
    protected List<TransactionWrapper> onLoading() {
        List<TransactionWrapper> list = solarisModule.listTx();
    ...
    protected BaseRecyclerAdapter<TransactionWrapper, ? extends BaseRecyclerViewHolder> initAdapter() {
        BaseRecyclerAdapter<TransactionWrapper, TransactionViewHolderBase> adapter = new BaseRecyclerAdapter<TransactionWrapper, TransactionViewHolderBase>(getActivity()) {
            ...
            protected void bindHolder(TransactionViewHolderBase holder, TransactionWrapper data, int position) {
                ...
                if (solarisRate!=null) {
                    localCurrency = solarisApplication.getCentralFormats().format(
                                    new BigDecimal(data.getAmount().getValue() * solarisRate.getRate().doubleValue()).movePointLeft(8)
                                    )
                                    + " " + solarisRate.getCode();
                ...
                holder.title.setText(getAddressOrContact(solarisModule,data));
                ...
                /*if (data.getOutputLabels()!=null && !data.getOutputLabels().isEmpty()){
                    ...
                    if (contact!=null) {
                        ...
                        holder.title.setText(data.getTransaction().getOutput(0).getScriptPubKey().getToAddress(solarisModule.getConf().getNetworkParams()).toBase58());
                    }
                }else {
                    holder.title.setText(data.getTransaction().getOutput(0).getScriptPubKey().getToAddress(solarisModule.getConf().getNetworkParams()).toBase58());
                }*/
