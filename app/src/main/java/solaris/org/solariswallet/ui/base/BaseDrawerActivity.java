...
import solaris.org.solariswallet.ui.contacts_activity.ContactsActivity;
import solaris.org.solariswallet.ui.donate.DonateActivity;
...
import static solaris.org.solariswallet.module.SolarisContext.OUT_OF_SYNC_TIME;
...
public class BaseDrawerActivity extends SolarisActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    private void checkState(){
        ...
        long lastBlockTime = solarisApplication.getAppConf().getLastBestChainBlockTime();
        if (lastBlockTime+OUT_OF_SYNC_TIME>now){
            // check if i'm syncing or i'm synched
            long peerHeight = solarisModule.getConnectedPeerHeight();
            if (peerHeight!=-1){
                if (solarisModule.getChainHeight()+10>peerHeight) {
            ...
            if (solarisModule.isAnyPeerConnected()) {
                long peerHeight = solarisModule.getConnectedPeerHeight();
                if (peerHeight!=-1){
                    if (solarisModule.getChainHeight()+10>peerHeight) {
                        blockchainState = BlockchainState.SYNC;
    ...
    private double calculateBlockchainSyncProgress() {
        long nodeHeight = solarisModule.getConnectedPeerHeight();
        if (nodeHeight>0){
            // calculate the progress
            // nodeHeight -> 100 %
            // current height -> x %
            return (solarisModule.getChainHeight()*100) / nodeHeight;
        }
        return -1;
    }
