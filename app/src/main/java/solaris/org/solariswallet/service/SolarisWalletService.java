package solaris.org.solariswallet.service;
...
import org.solarisj.core.listeners.AbstractPeerDataEventListener;
import org.solarisj.core.listeners.PeerConnectedEventListener;
import org.solarisj.core.listeners.PeerDataEventListener;
...
import solaris.org.solariswallet.module.SolarisModuleImp;
import solaris.org.solariswallet.module.store.SnappyBlockchainStore;
import solaris.org.solariswallet.rate.CoinMarketCapApiClient;
import solaris.org.solariswallet.rate.RequestSolarisRateException;
...
import static solaris.org.solariswallet.service.IntentsConstants.ACTION_ADDRESS_BALANCE_CHANGE;
import static solaris.org.solariswallet.service.IntentsConstants.ACTION_BROADCAST_TRANSACTION;
import static solaris.org.solariswallet.service.IntentsConstants.ACTION_CANCEL_COINS_RECEIVED;
import static solaris.org.solariswallet.service.IntentsConstants.ACTION_NOTIFICATION;
import static solaris.org.solariswallet.service.IntentsConstants.ACTION_RESET_BLOCKCHAIN;
import static solaris.org.solariswallet.service.IntentsConstants.ACTION_SCHEDULE_SERVICE;
...
import static solaris.org.solariswallet.service.IntentsConstants.NOT_BLOCKCHAIN_ALERT;
...
public class SolarisWalletService extends Service{
    ...
    private Logger log = LoggerFactory.getLogger(SolarisWalletService.class);

    private SolarisApplication solarisApplication;
    private SolarisModuleImp module;
    //private XlrtrumPeergroup pivtrumPeergroup;
    ...
    public class SolarisBinder extends Binder {
        public SolarisWalletService getService() {
            return SolarisWalletService.this;
        }
    }
        ...
        public void onBlocksDownloaded(final Peer peer, final Block block, final FilteredBlock filteredBlock, final int blocksLeft) {
            ...
            /*if (SolarisContext.IS_TEST)
                ...
                if (now - lastMessageTime > TimeUnit.SECONDS.toMillis(6)) {
                    ...
                    solarisApplication.getAppConf().setLastBestChainBlockTime(block.getTime().getTime());
    ...
    private class RunnableBlockChecker implements Runnable{
        ...
        @Override
        public void run() {
            org.solarisj.core.Context.propagate(SolarisContext.CONTEXT);
    ...
    private WalletCoinsReceivedEventListener coinReceiverListener = new WalletCoinsReceivedEventListener() {
        ...
        public void onCoinsReceived(Wallet wallet, Transaction transaction, Coin coin, Coin coin1) {
            ...
            org.solarisj.core.Context.propagate(CONTEXT);
            ...
            try {
                ...
                if (amount.isGreaterThan(Coin.ZERO)) {
                    ...
                    Intent resultIntent = new Intent(getApplicationContext(), SolarisWalletService.this.getClass());
                    ...
                    mBuilder = new NotificationCompat.Builder(getApplicationContext())
                            ...
                            .setColor(
                                    (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) ?
                                            ...
                                            :
                                            ContextCompat.getColor(SolarisWalletService.this, R.color.bgPurple))
    ...
    public void onCreate() {
        ...
        super.onCreate();
        try {
            log.info("Solaris service started");
            // Android stuff
            ...
            module = (SolarisModuleImp) solarisApplication.getModule();
            blockchainManager = module.getBlockchainManager();
            // connect to xlrtrum node
            /*pivtrumPeergroup = new XlrtrumPeergroup(solarisApplication.getNetworkConf());
            pivtrumPeergroup.addAddressListener(addressListener);
            module.setXlrtrumPeergroup(pivtrumPeergroup);*/
            ...
            String filename = SolarisContext.Files.BLOCKCHAIN_FILENAME;
            boolean fileExists = new File(file,filename).exists();
            blockchainStore = new SnappyBlockchainStore(SolarisContext.CONTEXT,file,filename);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        log.info("Solaris service onStartCommand");
        try {
            if (intent != null) {
                try {
    ...
    private void tryScheduleService() {
        ...
        if (!isSchedule){
            ...
            Intent intent = new Intent(this, SolarisWalletService.class);
    ...
    private void requestRateCoin(){
        final AppConf appConf = solarisApplication.getAppConf();
        SolarisRate solarisRate = module.getRate(appConf.getSelectedRateCoin());

        if (solarisRate==null || solarisRate.getTimestamp()+SolarisContext.RATE_UPDATE_TIME<System.currentTimeMillis()){
            new Thread(new Runnable() {
                @Override
                public void run() {
                    try {
                        CoinMarketCapApiClient c = new CoinMarketCapApiClient();
                        CoinMarketCapApiClient.SolarisMarket solarisMarket = c.getSolarisPxrice();
                        SolarisRate solarisRate = new SolarisRate("USD",solarisMarket.priceUsd,System.currentTimeMillis());

                        module.saveRate(solarisRate);
                        final SolarisRate solarisBtcRate = new SolarisRate("BTC",solarisMarket.priceBtc,System.currentTimeMillis());
                        module.saveRate(solarisBtcRate);

                        // Get the rest of the rates:
                        List<SolarisRate> rates = new CoinMarketCapApiClient.BitPayApi().getRates(new CoinMarketCapApiClient.BitPayApi.RatesConvertor<SolarisRate>() {
                            @Override
                            public SolarisRate convertRate(String code, String name, BigDecimal bitcoinRate) {
                                BigDecimal rate = bitcoinRate.multiply(solarisBtcRate.getRate());
                                return new SolarisRate(code,rate,System.currentTimeMillis());
                            }
                        });

                        for (SolarisRate rate : rates) {
                            module.saveRate(rate);
                        }

                    } catch (RequestSolarisRateException e) {
                        e.printStackTrace();
                    } catch (Exception e){
                        e.printStackTrace();
                    }
