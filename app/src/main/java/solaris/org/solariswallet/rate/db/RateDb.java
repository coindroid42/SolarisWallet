...
import solaris.org.solariswallet.contacts.AbstractSqliteDb;

/**
 * Created by mingxiao on 2018.3.19.
 */

public class RateDb extends AbstractSqliteDb<SolarisRate> {
    ...
    @Override
    protected ContentValues buildContent(SolarisRate obj) {
    ...
    @Override
    protected SolarisRate buildFrom(Cursor cursor) {
        ...
        return new SolarisRate(coin,value,timestap);
    }
    ...
    public void insertOrUpdateIfExist(SolarisRate solarisRate) {
        if (getRate(solarisRate.getCode())==null){
            insert(solarisRate);
        }else {
            updateByKey(KEY_COIN,solarisRate.getCode(),solarisRate);
        }
    }
}
