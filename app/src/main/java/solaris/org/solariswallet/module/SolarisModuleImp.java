...
import solaris.org.solariswallet.module.wallet.WalletBackupHelper;
...
public class SolarisModuleImp implements SolarisModule {

    private static final Logger logger = LoggerFactory.getLogger(SolarisModuleImp.class);

    private ContextWrapper context;
    private WalletConfiguration walletConfiguration;
    private WalletManager walletManager;
    private BlockchainManager blockchainManager;
    private XlrtrumPeergroup peergroup;
    private ContactsStore contactsStore;
    private RateDb rateDb;

    // cache balance
    private long availableBalance = 0;
    private BigDecimal xlrInUsdHardcoded = new BigDecimal("1.5");

    public SolarisModuleImp(ContextWrapper contextWrapper, WalletConfiguration walletConfiguration,ContactsStore contactsStore,RateDb rateDb) {
    ...
    // todo: clean this.
    //public void setXlrtrumPeergroup(XlrtrumPeergroup peergroup){
    ...
    public BigDecimal getAvailableBalanceLocale() {
        return xlrInUsdHardcoded.multiply(new BigDecimal(availableBalance));
    }
    ...
    public Transaction completeTx(Transaction transaction,Address changeAddress,Coin feePerKb) throws InsufficientMoneyException {
        ...
        if (transaction.getInputs()!=null && !transaction.getInputs().isEmpty()){
            ...
            sendRequest.coinSelector = new solaris.org.solariswallet.module.wallet.DefaultCoinSelector(unspent);
        }
        sendRequest.signInputs = true;
        sendRequest.shuffleOutputs = false; // don't shuffle outputs to know the contact
        if (changeAddress!=null){
    ...
    public Transaction completeTxWithCustomFee(Transaction transaction,Coin fee) throws InsufficientMoneyException{
        ...
        walletManager.completeSend(sendRequest);
        // if the fee is different to the custom fee and the tx size is lower than 1kb (1000 bytes in solaris core)
    ...
    public SolarisRate getRate(String coin) {
   ...
   public boolean isSyncWithNode() throws NoPeerConnectedException {
    ...
    @Override
    public List<SolarisRate> listRates() {
        return rateDb.list();
    }
    ...
    public void saveRate(SolarisRate solarisRate){
        rateDb.insertOrUpdateIfExist(solarisRate);
    }
}
